...
- (void) dealloc
{
	...
	MCLog( @"TSAlertView: TSAlertOverlayWindow dealloc" );
...
- (void) dealloc
{
	MCLog( @"TSAlertView: TSAlertViewController dealloc" );
	[super dealloc];
}
...
- (id)initWithTitle:(NSString *)t view:(UIView *)v delegate:(id)d cancelButtonTitle:(NSString *)cancelButtonTitle otherButtonTitles:(NSString *)otherButtonTitles, ...
{
    if ( (self = [super init] ) ) // will call into initWithFrame, thus TSAlertView_commonInit is called
	{
		self.title = t;
...
- (NSInteger) addButtonWithTitle: (NSString *) t
{
    ...
    b.titleLabel.font = [UIFont boldSystemFontOfSize:17.0];
...
- (CGSize) recalcSizeAndLayout: (BOOL) layout
{
	...
	CGFloat totalHeight = kTSAlertView_TopMargin + titleLabelSize.height + kTSAlertView_RowMargin + _messageView.frame.size.height + kTSAlertView_RowMargin + messageViewSize.height + inputRowHeight + kTSAlertView_RowMargin + buttonsAreaSize.height + kTSAlertView_BottomMargin;
	...
	if ( layout )
	{
        ...
        if( _messageView !=nil )
        {
            _messageView.frame = CGRectMake( kTSAlertView_LeftMargin, y, _messageView.frame.size.width, _messageView.frame.size.height);
            [self addSubview: _messageView];
            y += _messageView.frame.size.height + kTSAlertView_RowMargin;
        }
